...
$query=mysql_query("SELECT * FROM comentario,articulo WHERE comentario.id=$id AND articulo.id=comentario.articulo_id");
$get=mysql_fetch_array($query);

echo "<form action='alterar.php' method='post'>
			<input type='hidden' value='$id' name='id'>
			<h1>Usted está editando el comentario de $get[2] que se encuentra en el artículo $get[11]</h1>
			<br/>
			<p>Autor:</p><input type='text' value='$get[1]' name='autor'>
			<p>Texto:</p><textarea name='texto'>$get[4]</textarea>
			<p>Fecha de publicación:</p>\n".substr($get[6],11,8)."\n".substr($get[6],8,2)."-".substr($get[6],5,2)."-".substr($get[6],0,4);

			echo "<p>Estado actual: $get[5]</p>
			<p>Cambiar estado:</p>
			<select name='estado'>
