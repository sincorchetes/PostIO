...
$query_user=mysql_query("SELECT * FROM usuario,usuario_tiene_rol WHERE usuario.id=$_SESSION[id_nick] AND usuario_tiene_rol.usuario_id=usuario.id;");
...
while ($data=mysql_fetch_array($query_user)){
			...
			<p>Nombre:</p><input type='text' value='$data[3]' name='nombre'>
			<p>Apellidos:</p><input type='text' value='$data[4]' name='apellidos'>
			<p>Fecha de Nacimiento:</p><input type='text' value='$data[5]' name='fecha_nac' id='datepicker'>
			<p>Nacionalidad:</p><input type='text' value='$data[6]' name='nacionalidad'>
			<p>E-mail</p><input type='text' value='$data[7]' name='email'>
			<p>URL, blog</p><input type='text' value='$data[8]' name='url'>
			<p>Sobre mi:</p><textarea name='sobre'>$data[9]</textarea>
			<input type='hidden' name='estado' value='$data[12]'>
			<br/><br/>
