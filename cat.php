...
echo "<html>
		...
		echo "<div id='main'>";
			...
			$id=$_REQUEST['id'];
			$get_data=mysql_query("SELECT * FROM articulo,categoria WHERE articulo.categoria_id=$id AND articulo.estado='Publicado' AND articulo.categoria_id=categoria.id AND categoria.estado='Activada' ORDER BY articulo.fecha_publicacion DESC LIMIT 0,50;");
			echo "<table>
					<tr>
						<td>Título</td>
						<td>Texto</td>
			...
			while($gd=mysql_fetch_array($get_data)){
				echo "<tr>
						<td><a href='show.php?id=$gd[0]'>$gd[1]</a></td>
						<td>".substr($gd[2],0,150)."</td>";
						echo "<td>".substr($gd[5],11,8)."<br/>".substr($gd[5],8,2)."-".substr($gd[5],5,2)."-".substr($gd[5],0,4)."</td>
						</tr>";
				}
				echo "</table>";
